...
public class ServiceAuditAspect {
    ...
    public void afterDeleteOrRemove(JoinPoint joinPoint) {
        Long userId = getUserIdFromRequest();
        ...
        auditLogClient.log(
            ...
            resourceType,
            resourceId,
            buildDetails("DELETE", resourceType, resourceId, joinPoint, null)
        );
    }
    ...
    private String buildDetails(String action, String resourceType, Long resourceId, JoinPoint joinPoint, Object result) {
        ...
        if (args != null && args.length > 0 && args[0] != null) {
            // WHY: keep a lightweight payload snapshot for compliance traceability.
            details.append(" | payload=").append(String.valueOf(args[0]));
        } else if (result != null) {
            // WHY: when input args are empty, capture returned payload context.
            details.append(" | payload=").append(String.valueOf(result));
        }
