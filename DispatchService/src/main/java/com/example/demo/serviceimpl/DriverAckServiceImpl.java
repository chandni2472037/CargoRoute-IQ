...
public class DriverAckServiceImpl implements DriverAckService {
    ...
    public DriverAckResponseDTO fetchByDispatchID(Long dispatchID) {
        ...
        DriverAck ack = driverAckRepository
                .findByDispatch_DispatchID(dispatchID);

        if (ack == null) {
            throw new ResourceNotFoundException(
                    "DriverAck not found for dispatchID: " + dispatchID
            );
        }
    ...
    public DriverAckResponseDTO fetchByDriverID(Long driverID) {
        ...
        DriverAck ack = driverAckRepository
                .findByDriver_DriverID(driverID);

        if (ack == null) {
            throw new ResourceNotFoundException(
                    "DriverAck not found for driverID: " + driverID
            );
        }
